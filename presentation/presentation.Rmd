...
all.nest.count %>%
  ...
  theme_bw() +
    labs(y = "Total Number of Nesting Gulls", x = "Year")
```
...
all.nest.count %>%
  ...
  geom_col(aes(x = year, y = n), fill = "#9EC2FF") +
  theme_bw() +
    labs(y = "Total Number of Nesting Great Black-backed Gulls", x = "Year")
```
```{r fig.height=2, fig.width=5}
all.nest.count %>%
  group_by(year, species) %>%
  ...
  geom_col(aes(x = year, y = n), fill = "#9EC2FF") +
  theme_bw() +
    labs(y = "Total Number of Nesting Common Eiders", x = "Year")
```
...
animation.points.only <- ggplot(gullshapecombined) +
  ...
  scale_fill_manual(values = c("#DBE9F5", "#9EC2FF"))
...
# anim_save(animate.save, filename = "colony_animation.gif")
```
...
nest_density %>%
  ...
  ggplot() +
  geom_bar(aes(x = year, fill = habitat)) +
...
nest_density %>%
  ...
  group_by(year) %>%
  summarise(median = median(distance), se = se(distance)) %>%
  ggplot() +
  geom_col(aes(x = year, y = median, group = year), fill = "#9EC2FF") +
  geom_errorbar(aes(x = year, ymin = (median - se), ymax = (median + se)), size = 0.5, width = 0.5)

#### ELEANOR I CAN"T FIGURE THIS OUT. GRUMBLE!
nest_density %>%
  filter(distance_rank == 1) %>%
  ...
  summarise(median = median(distance), se = se(distance)) %>%
  ggplot() +
  geom_col(aes(x = year, y = median, fill = habitat), position = position_dodge(preserve = "single"), width = 0.75) +
  scale_fill_manual(values = c("#4259C3", "#9EC2FF")) +
  geom_errorbar(aes(x = year, ymin = (median - se), ymax = (median + se), group = habitat), size = 0.5, width = 0.5, position = position_dodge(preserve = "single", width = 0.75))

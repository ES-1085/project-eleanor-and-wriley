...
splitcomments <- function(inputdataframe) {
  for (i in 1:length(colnames(inputdataframe))){
    if("puffin" %in% colnames(inputdataframe)[i]) {
    #puffins
  mutate(inputdataframe, puffin = str_extract(other, "\\d*puff|\\d*.puff|\\d*atpu|\\d*.atpu|\\d*puffis|\\d*puffins|\\d*.puffins|\\d*.ATPU|\\d*.ALPU|\\d*.puffin|\\d*..ATPU")) %>% 
  mutate(puffin = as.double(str_extract(puffin, "\\d*")))
    } else if ("dcco" %in% (colnames(inputdataframe)[i])) {
    #cormorants
  mutate(inputdataframe, dcco = str_extract(other, "\\d*corm|\\d*dcc|\\d*dcco|\\d*ddco|\\d*.DCCO|\\d*.DCC0|\\d*.commorants|\\d*.corm|\\d*.cormorants|d*..corm|\\d*.dcco")) %>% 

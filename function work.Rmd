...
splitcomments <- function(inputdataframe) {
    mutate(puffin = ifelse(puffins %in% colnames(inputdataframe), puffin, str_extract(other, "\\d*puff|\\d*.puff|\\d*atpu|\\d*.atpu|\\d*puffis|\\d*puffins|\\d*.puffins|\\d*.ATPU|\\d*.ALPU|\\d*.puffin|\\d*..ATPU")))
  return(inputdataframe)
}
```

...
#NOTE: Wriley, to read in data, find the folder that this project made on your computer, go to the proposal folder within that folder, and plop your data files into the data folder within the proposal folder:
#filepath: initialfilejunk/project-eleanor-and-wriley/proposal/data
#then you can read in data with this syntax:
...
An interesting visualization of 2022 petrel survey data, where the number of burrows per survey plot is mapped over longitude and latitude. The bimodal distribution in longitude is very interesting to us. 
```{r petrel exploratory visualization, echo = FALSE}
#Eleanor visualize some simple petrel data
#will probably be burrows/subplot mapped over latitude to show their bimodal distribution
#will also do some interesting summary statistics for the population estimate perhaps
#what else for an initial analysis? Could do my barplot of past petrel counts or something
#And if you plop some gull data in the data folder I can play with that too
```

An example of a visualization from 2022 gull data, looking at fledging success by habitat:

```{r reading gull data, echo = FALSE}
#Wriley visualize some simple gull data
library(readr)
#chickcheck2022 <- read_csv("/Users/wrileyhodge/Wriley and Eleanor Final Project/data/chickchecktidy2022.csv") #the only way I can get this to read in is by typing out my whole path....
chickcheck2022 <- read_csv("data/chickchecktidy2022.csv")
#fledging2022 <- read_csv("/Users/wrileyhodge/Wriley and Eleanor Final Project/data/fledging_final2022.csv")
fledging2022 <- read_csv("data/fledging_final2022.csv")
```


```{r gull exploratory visualization, echo = FALSE}
#I'm so annoyed I literally can't figure out how to make this graph in ggplot and I'm sad
barfledge=tapply(fledging2022$fledging_success,fledging2022$habitat,mean)
bar1=barplot(barfledge,xlab="Habitat",ylab="Fledging Success",ylim=c(0,3),col="darkslategray3", main="Fledging Success in Each Habitat")

se=function(x)
{y = x[!is.na(x)] # remove the missing values
sqrt(var(as.vector(y))/length(y))} # calculate SE
plot.se=tapply(fledging2022$fledging_success, fledging2022$habitat, se)

plot.err=tapply(fledging2022$fledging_success, fledging2022$habitat,sd)
nlo=barfledge-plot.se; nhi=barfledge+plot.se
segments(x0=bar1, x1=bar1, y0=nlo, y1=nhi) # bars
segments(x0=bar1-.1, x1=bar1+0.1, y0=nhi, y1=nhi) # lower whiskers
segments(x0=bar1-.1, x1=bar1+0.1, y0=nlo, y1=nlo) # upper whiskers
...
#I think you have to use geom_col()
...
fledging2022 %>% 
  ...
  labs(caption = "Error Bars Denote Standard Error")

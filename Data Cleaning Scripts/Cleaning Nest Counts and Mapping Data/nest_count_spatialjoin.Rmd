...
animation.points.only <- ggplot(gullshapetest) +
  geom_sf(data = outline, fill = "lightblue")+
  geom_sf(data = gullshapetest, color = "midnightblue", aes(group = year), size = .8) +
...
```{r}
animate.save <- animate(animation.points.only, nframes = 48, duration = 40)

anim_save(animate.save, filename = "anim3.gif")
```

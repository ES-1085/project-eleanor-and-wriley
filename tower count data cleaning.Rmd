...
tower_count_2000 <- tower_count_2000 %>% 
  mutate(coei_ad = as.double(str_extract(`# eiders`, "\\d*"))) %>% 
  mutate(coei_ju = as.double(str_extract(`# eiders`, "(?<=\\().*(?=\\))"))) %>% 
  mutate(herg = as.double(`# gulls`), blgu = as.double(`# squees`), other = others)

tower_count_2000 <- cleaner(tower_count_2000)

tower_count_2000 <- tower_count_2000 %>% 
  pivot_longer(cols = c(herg, coei_ad, coei_ju, blgu, puffin, dcco, ternsp, razo, lagu, comu, baea, noga, corvid, peepsp, seals, colo, cago, porpoise),
               names_to = "species",
               values_to = "count")

tower_2000_clean <- fixcolumns(tower_count_2000)

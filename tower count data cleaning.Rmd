...
splitcomments <- function(inputdataframe) {  
  ...
  mutate(corvid = as.double(str_extract(corvid, "\\d*"))) %>%
    #sandpipers
  mutate(peepsp = str_extract(other, "\\d*.SPSA|\\d*.WHIM|\\d*..SPSA|\\d*.spotted sandpiper|\\d*.least sandpiper")) %>% 
  mutate(peepsp = as.double(str_extract(peepsp, "\\d*"))) %>% 
    #seals
  mutate(seals = str_extract(other, "\\d*.Seal|\\d*.Seals|\\d*.Harbor|\\d*.Gray|\\d*.seals|\\d*.grey|\\d*.seal|\\d*.harbor|\\d*.gray")) %>% 

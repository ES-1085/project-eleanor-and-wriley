...
splitcomments <- function(inputdataframe) {  
  r <- inputdataframe %>% 
    #puffins
  mutate(puffin = str_extract(other, "\\d*puff|\\d*.puff|\\d*atpu|\\d*.atpu|\\d*puffis|\\d*puffins|\\d*.puffins|\\d*.ATPU|\\d*.ALPU|\\d*.puffin")) %>% 
  ...
  mutate(corvid = as.double(str_extract(corvid, "\\d*"))) %>%
    #sandpipers
  mutate(peepsp = str_extract(other, "\\d*.SPSA|\\d*.WHIM|\\d*..SPSA|\\d*.spotted sandpiper|\\d*.least sandpiper|\\d*.pipers")) %>% 
  mutate(peepsp = as.double(str_extract(peepsp, "\\d*"))) %>% 
    #seals
  mutate(seals = str_extract(other, "\\d*.Seal|\\d*.Seals|\\d*.Harbor|\\d*.Gray|\\d*.seals|\\d*.grey|\\d*.seal|\\d*.harbor|\\d*.gray|\\*.Grey|\\d*.Grey Seal|\\d*.Grey Seals")) %>% 
  ...
  mutate(cago = as.double(str_extract(cago, "\\d*"))) %>% 
    #porpoise
  mutate(porpoise = str_extract(other, "\\d*.Harbor|\\d*.porpoise|\\d*.Porpoises")) %>% 
  ...
  mutate(gbhe = as.double(str_extract(gbhe, "\\d*"))) %>% 
    #scoter
  mutate(scoter = str_extract(other, "\\d*.scoter|\\d*.Scoter")) %>% 
...
colnames(tower_count_2012) = c("date","herg","gbbg","blgu","coei_ad","coei_ju","puffin","razo","dcco","corvid","corvid2","colo","noga","grco","baea","lagu","peepsp","ternsp","Comments")
...
```{r 2013}
tower_count_2013 <- read_excel("data/tower_count/tower_count_2013.xls", skip = 1)
tower_count_2013 <- tower_count_2013[3:82,]
colnames(tower_count_2013) = c()
```

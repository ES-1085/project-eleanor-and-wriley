...
#there is a way to update this so that it leaves existing columns alone. In short, there should be a way to tell it to loop through the column names in a dataframe and not create a new column if the column already exists. Probably an if_else statement.
splitcomments <- function(inputdataframe) {
  ...
  mutate(corvid = as.double(str_extract(corvid, "\\d*"))) %>%
    #sandpipers
  mutate(peepsp = str_extract(other, "\\d*.SPSA|\\d*.WHIM|\\d*..SPSA|\\d*.spotted sandpiper|\\d*.least sandpiper|\\d*.pipers|\\d*.common sandpipers|\\d*.piper|\\d*.red knots|\\d*.wimbrel|\\d*.spotted sandpipers|\\d*.sandpiper|\\d*.sandpipers")) %>% 
  ...
  mutate(porpoise = as.double(str_extract(porpoise, "\\d*"))) %>% 
    #raptors
  mutate(raptor = str_extract(other, "\\d*.tv|\\d*.peregrine|\\d*.PEFA")) %>% 
  ...
  mutate(gbhe = as.double(str_extract(gbhe, "\\d*"))) %>% 
    #scoter
  mutate(scoter = str_extract(other, "\\d*.scoter|\\d*.Scoter|\\d*.BLSC|\\d*.surf scoter")) %>% 
...
tower_count_2011 <- tower_count_2011 %>% 
  ...
  mutate(ATPU = paste(ATPU, "ATPU")) %>% 
  mutate(Others = paste(Others, ATPU, sep = ",")) %>% #good way to fix puffin columns
  select(-ATPU, -Comments)
...
tower_count_2017 <- read_excel("data/tower_count/tower_count_2017.xlsx") %>% 
  ...
  mutate(coei_ju = as.double(str_extract(coei_ju, "\\d*$"))) %>% #I am a regex genuis now
  select(-`COEI + chix`)
...
##Need to find 2020, update 2021 to include incidental species, and enter 2022 data
```{r}
tower <- rbind(tower_2000_clean,tower_2001_clean,tower_2002_clean,tower_2003_clean,tower_2005_clean,tower_2006_clean,tower_2007_clean,tower_2008_clean,tower_2009_clean,tower_2010_clean,tower_2011_clean,tower_2012_clean,tower_2013_clean,tower_2014_clean,tower_2015_clean, tower_2016_clean,tower_2017_clean,tower_2018_clean,tower_2019_clean, tower_2020_clean)
